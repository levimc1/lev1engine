...
namespace taskm {
  ...
  namespace internal {
    ...
    struct Flag {
      ...
      static Flag make_default() {
        Flag f;
        static_assert(sizeof(T) <= game::settings::MAX_FLAG_BYTES, "A típus túl nagy, növeld a MAX_FLAG_BYTES-ot!");
      ...
      template<typename T>
      T& get() {
        static_assert(sizeof(T) <= game::settings::MAX_FLAG_BYTES, "A típus túl nagy, növeld a MAXname_FLAG_BYTES-ot!");
    ...
    struct Address {
      ...
      Address(std::initializer_list<T> values) : size(static_cast<uint8_t>(values.size())) {
        ...
        if (values.size() * sizeof(T) > game::settings::MAX_ADDRESS_SIZE) {
  ...
  F& flag(std::initializer_list<T> values) {
    ...
    if (it == internal::flags.end()) {
      it = internal::flags.insert({address, Flag::make_default<F>()}).first; // Alap, nem tudom mi lehet ez... 
    }
